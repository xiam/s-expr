...
func TestParserEvaluate(t *testing.T) {
	...
	Defn("defn", func(ctx *Context) error {
		...
		if err := ctx.Parent.Set(name.raw(), wrapperFn); err != nil {
			return err
		}
		log.Printf("FOO 3")
	...
	Defn("set", func(ctx *Context) error {
		...
		log.Printf("ctx: %v, name: %v, value: %v", ctx, name, value)

		ctx.Parent.Set(name.raw(), value)
		ctx.Yield(True)
